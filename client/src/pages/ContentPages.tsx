...
export function Blog() {
  return <SiteFrame><Seo title="Cerebral Palsy Blog & Caregiver Learning in Kenya | KindStepCare" description="SEO-rich, plain-language articles about cerebral palsy, home therapy, caregiver learning, inclusion, and family support in Kenya and Africa." path="/blog" /><PageHero eyebrow="The KindStepCare journal" title="Useful knowledge for the next small step." copy="Thoughtful, plain-language articles about cerebral palsy care, therapy, play, caregiver confidence, and inclusion." image="/manus-storage/hero-family-play_4fa9122a.jpg" /><main className="container py-16"><div className="grid gap-6 lg:grid-cols-3">{posts.map((post) => <article key={post.slug} className="group overflow-hidden rounded-[2rem] border border-[#d9e9e5] bg-white shadow-[0_16px_50px_rgba(26,42,38,.05)]"><img src={post.image} alt="" className="h-56 w-full object-cover transition duration-700 group-hover:scale-105" /><div className="p-7"><div className="eyebrow">{post.category}</div><h2 className="mt-4 font-serif text-3xl font-semibold">{post.title}</h2><p className="mt-4 leading-7 text-[#607a74]">{post.description}</p><a href={`/blog/${post.slug}`} className="mt-6 inline-flex font-bold text-[#0d7a6b]">Read the guide <span className="ml-2">→</span></a></div></article>)}</div></main></SiteFrame>;
}
