...
export async function listAllContent() { const db = await getDb(); if (!db) return []; return db.select().from(contentItems).orderBy(desc(contentItems.updatedAt)); }
export async function createContentItem(input: { slug: string; title: string; content: string; category: string; imageUrl?: string; published?: number }) { const db = await getDb(); if (!db) return { success: false }; await db.insert(contentItems).values({ ...input, published: input.published ?? 1 }); return { success: true }; }
